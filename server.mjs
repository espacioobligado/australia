        ...
        try {
          ...
          while (userExists) {
              ...
              if (!userExists) {
                  ...
                  // res.setHeader('Set-Cookie', `usuario=${nuevoUsuario}; Path=/`);
  ...
  io.on('connection', async (socket) => {
    console.log('conectado '); 
      ...
      socket.on("usuariosEnChat", async msg => {
        ...
        room = msg
        const remitente = partes[0]; const publicacionDeUsuario = partes[1];      
        parseInt(remitente);               parseInt(publicacionDeUsuario)
          const chatIdCreation = async () => {
            const currentDate = new Date();
            if (remitente && publicacionDeUsuario) {                                                                   
                let query = 'SELECT * FROM chat WHERE remitente = $1 AND publicacionDeUsuario = $2'; let values = [remitente,publicacionDeUsuario];
                let result = await conn.query(query, values);
                if (result.rows.length !== 0) {
                  chat_id = result.rows[0].id;
                  let updateQuery = 'UPDATE chat SET activo = $1 WHERE id = $2';
                  let values3 = [true,chat_id]; await conn.query(updateQuery, values3);
                ...
                console.log('chat_id1', chat_id)
                  ...
                  values.push(true, currentDate); await conn.query(query, values);
                  let query2 = 'SELECT * FROM chat WHERE remitente = $1 AND publicacionDeUsuario = $2'; let values2 = [remitente,publicacionDeUsuario];
